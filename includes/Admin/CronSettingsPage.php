...
class CronSettingsPage
{
    ...
    {
        add_action('admin_notices', [$this, 'displayCronInstructions']);
    }
     ...
     */
    public function displayCronInstructions(): void
    {
...
<?php
    ...
    private function checkCronStatus(string $hookName, int $expectedInterval, int $offsetMinutes): string
    {
        ...
        if ($nextRun) {
            return sprintf(
                __('Next run for %s is scheduled at %s.', 'ai-sports-writer'),
                $hookName,
                date('Y-m-d H:i:s', $nextRun)
            );
        } else {
            return sprintf(
                __('No scheduled run found for %s.', 'ai-sports-writer'),
                $hookName
            );
        }
    }
